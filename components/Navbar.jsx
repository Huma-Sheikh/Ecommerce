    ...
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-32 py-3 border-b border-gray-300 text-gray-700">
      ...
      <ul className="hidden md:flex items-center gap-4 ">
       ...
       <UserButton>
        ...
        <UserButton.MenuItems>
          <UserButton.Action label= "MyOrders" labelIcon={<BagIcon></BagIcon> }onClick={()=> router.push("/my-orders")}></UserButton.Action>
        </UserButton.MenuItems>
       </UserButton>
       </>
      ...
      <div className="flex items-center md:hidden gap-3">
        {isSeller && <button onClick={() => router.push('/seller')} className="text-xs border px-4 py-1.5 rounded-full">Seller Dashboard</button>}
        <button onClick={openSignIn} className="flex items-center gap-2 hover:text-gray-900 transition"> </button>
          <Image src={assets.user_icon} alt="user icon" />
          { 
       user ? <>
       <UserButton>
